...
class PipeRenderer:
    ...
    def draw(self, screen, pipes, theme_name):
        ...
        body_surface, cap_surface = self._get_surfaces(colors)
        ...
        for pipe in pipes:
            ...
            self._draw_pipe_segment(screen, body_surface, cap_surface, int(pipe.x), top_height, True)
            self._draw_pipe_segment(screen, body_surface, cap_surface, int(pipe.x), bottom_height, False, bottom_y)
    ...
    def _build_pipe_surfaces(self, colors):
        ...
        return body_surface, cap_surface

    def _draw_pipe_segment(self, screen, body_surface, cap_surface, x, height, is_top, y=0):
        ...
        if is_top:
            ...
            flipped_cap = pygame.transform.flip(cap_surface, False, True)
            screen.blit(flipped_cap, (cap_x, max(0, height - cap_height)))
            return
